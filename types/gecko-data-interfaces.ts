...
export interface CoinMarketData extends BasicCoin {
  image?: string;
...
export interface CoinListResponse extends BasicCoin {
  platforms?: {
    [p: string]: string;
  };
}
...
export interface ExchangeIdData {
  name: string;
  year_established: number;
  country: null;
  description: string;
...
export type CompanyHoldingsData = {
  name: string;
  symbol: string;
  country: string;
...
export interface TopHoldingsResponse {
  ...
  companies: CompanyHoldingsData[];
}

export interface AssetPlatformData {
  id: string;
